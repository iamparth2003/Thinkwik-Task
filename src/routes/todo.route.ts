...
import { createTodoSchema, getTodosQuerySchema, updateTodoSchema } from "../validators/todo.validator";
import { validateRequest } from "../middleware/requestValidator";

const router = Router();

router.post("/", authMiddleware, validateRequest(createTodoSchema), createTodoItem);
router.get("/", authMiddleware, validateRequest(getTodosQuerySchema),getTodoItems);
